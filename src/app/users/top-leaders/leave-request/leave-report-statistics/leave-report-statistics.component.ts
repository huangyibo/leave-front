...
export class TopLeaderLeaveReportStatisticsComponent implements OnInit, OnDestroy {
  ...
  getDepartments() {
    this.departmentService.list().subscribe(
      data => {
        ...
        let allDepartment = new Department();
        allDepartment.id = null;
        allDepartment.name = '全部';
        this.departments.push(allDepartment);
      }
    );
  }

  getUserTypes(){
    UserType.listUserType().subscribe(
      data=>{
        this.userTypes = data;
      }
    );
